...
export async function fetchTopicos(): Promise<Topico[]> {
  const res = await fetch("https://el-club-del-frijol.onrender.com/api/v1/topicos");
...
export async function fetchTopicoPorId(idTopico: string): Promise<Topico> {
  const res = await fetch(`https://el-club-del-frijol.onrender.com/api/v1/topicos/${encodeURIComponent(idTopico)}`);
  if (!res.ok) {
...
export async function actualizarTopico(
  ...
  const res = await fetch(`https://el-club-del-frijol.onrender.com/api/v1/topicos/${encodeURIComponent(id)}`, {
    method: "PUT",
