...
router.get("/google/callback", passport_1.default.authenticate("google", { session: false }), (req, res) => {
    ...
    const frontendCallbackUrl = process.env.URL_FRONTEND || "http://localhost:5173";
    const front = new URL(frontendCallbackUrl);
    front.searchParams.set('token', token);
    front.searchParams.set('usuario', encodeURIComponent(JSON.stringify({
